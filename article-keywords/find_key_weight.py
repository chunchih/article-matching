...
model = models.Word2Vec.load_word2vec_format('../word2vec-model/med250.model.bin',binary=True)
...
for i in range(len(s_acc)):
	...
	for j in range(i+1,len(s_acc)):
		if fg[j] != 1:
			if model.similarity(s_acc[i][0], s_acc[j][0]) >= 0.65:
				fg[j] = 1
...
with io.open(sys.argv[1][:-8]+"_keywords.txt",'w',encoding='utf-8') as output:
